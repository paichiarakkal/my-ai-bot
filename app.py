...
if not st.session_state.auth:
    ...
    if page == "📊 Advisor":
        ...
        # --- പൂജ്യങ്ങൾ മാറ്റാനും കളർ വരാനുമുള്ള സെക്ഷൻ ---
        def highlight_cols(x):
            style_df = pd.DataFrame('', index=x.index, columns=x.columns)
            style_df.loc[pd.to_numeric(x['Debit'], errors='coerce') > 0, 'Debit'] = 'background-color: #ffe6e6; color: red; font-weight: bold;'
            style_df.loc[pd.to_numeric(x['Credit'], errors='coerce') > 0, 'Credit'] = 'background-color: #e6ffed; color: green; font-weight: bold;'
            return style_df

        # നമ്പറുകൾ ഫോർമാറ്റ് ചെയ്ത് സ്റ്റൈൽ അപ്ലൈ ചെയ്യുന്നു
        styled_df = df.iloc[::-1].style.apply(highlight_cols, axis=None).format({'Debit': "{:.2f}", 'Credit': "{:.2f}"}, na_rep="")
