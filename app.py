...
<style>
    ...
    .sidebar-chart-link:hover { background: #BF953F; color: #000 !important; }
...
st_autorefresh(interval=30000, key="faisal_total_final_v1")
...
if mode == "MARKET":
    ...
    # 🎯 നിന്റെ GitHub-ലെ ഫോട്ടോ ഇവിടെ വരുന്നു
    my_photo_url = "https://raw.githubusercontent.com/paichiarakkal/my-ai-bot/main/image_7.png" 
    
    st.markdown(f"""
    <div style="text-align: center; margin-top: 30px; margin-bottom: 20px;">
        <img src="{my_photo_url}" alt="Trader Faisal" 
             style="width: 200px; height: 200px; border-radius: 50%; border: 6px solid #FFD700; 
             box-shadow: 0px 4px 20px rgba(0,0,0,0.6); object-fit: cover;">
    </div>
    """, unsafe_allow_html=True)
    
    st.markdown("<h3 style='text-align: center; color: #000;'>Happy Trading, Faisal!</h3>", unsafe_allow_html=True)
    ...
    with st.expander("പുതിയ ട്രേഡ് ചേർക്കാം", expanded=True):
        ...
        if st.button("SAVE TRADE"):
            ...
            st.success(f"ട്രേഡ് സേവ് ചെയ്തു! P&L: ₹{pnl}")
            st.rerun()
    ...
    if os.path.isfile(FILE_NAME):
        ...
        st.subheader("P&L Performance Graph")
        st.bar_chart(df.set_index('Date')['P&L'])
    else:
        st.info("ട്രേഡുകൾ ഒന്നും രേഖപ്പെടുത്തിയിട്ടില്ല.")
