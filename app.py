import streamlit as st
import yfinance as yf
import pandas as pd
import plotly.graph_objects as go
import time

# --- Page Config ---
st.set_page_config(page_title="Faisal's AI Terminal", layout="wide")

# --- 1. വാലറ്റ് ബാലൻസ് ---
if 'balance' not in st.session_state:
    st.session_state.balance = 471435.50 #

# --- 2. ലൈവ് ഡാറ്റാ ഫംഗ്‌ഷൻ ---
def get_exchange_rates():
    try:
        inr_data = yf.download("INR=X", period="1d", progress=False)
        aed_data = yf.download("AED=X", period="1d", progress=False)
        gold_data = yf.download("GC=F", period="1d", progress=False)
        
        # MultiIndex ഒഴിവാക്കാൻ അവസാനത്തെ വാല്യൂ മാത്രം എടുക്കുന്നു
        inr = float(inr_data['Close'].values[-1])
        aed = float(aed_data['Close'].values[-1])
        gold = float(gold_data['Close'].values[-1])
        return inr, aed, gold
    except:
        return 83.30, 3.67, 2300.0

# --- 3. AI അസിസ്റ്റന്റ് ലോജിക് (Fixed for Series/Float errors) ---
def get_ai_advice(rsi_val, price, ema_val):
    try:
        rsi = float(rsi_val)
        ema = float(ema_val)
        
        if rsi < 35:
            return "🔥 AI BUY SIGNAL: മാർക്കറ്റ് താഴെയാണ്, വാങ്ങാൻ നല്ല സമയം!", "#00FFA3", "#003322"
        elif rsi > 65:
            return "⚡ AI SELL SIGNAL: വില ഉയർന്നിട്ടുണ്ട്, ഇപ്പോൾ വിൽക്കാം!", "#FF3131", "#330000"
        elif price > ema:
            return "📈 TREND: മാർക്കറ്റ് Uptrend-ലാണ്. ഹോൾഡ് ചെയ്യാം.", "#00D1FF", "#002B36"
        return "⚖️ MARKET WATCH: പുതിയ അവസരത്തിനായി ബോട്ട് നോക്കുന്നു...", "#FFD700", "#332B00"
    except:
        return "⚖️ SYNCING: ഡാറ്റ അപ്‌ഡേറ്റ് ചെയ്യുന്നു...", "#FFD700", "#332B00"

# --- 4. മെയിൻ ഡിസ്‌പ്ലേ ലൂപ്പ് ---
st.sidebar.title("🤖 Faisal AI Control")
asset_choice = st.sidebar.selectbox("Select Asset", ["Gold (Live)", "Nifty 50", "Crude Oil"])

placeholder = st.empty()

while True:
    with placeholder.container():
        ticker_map = {"Gold (Live)": "GC=F", "Nifty 50": "^NSEI", "Crude Oil": "CL=F"}
        # ഡാറ്റ ഡൗൺലോഡ് ചെയ്യുന്നു
        df = yf.download(ticker_map[asset_choice], period="1d", interval="1m", progress=False)
        
        if not df.empty:
            # MultiIndex പ്രശ്നം ഒഴിവാക്കാൻ കോളം പേരുകൾ ലളിതമാക്കുന്നു
            if isinstance(df.columns, pd.MultiIndex):
                df.columns = df.columns.get_level_values(0)
            
            last_p = float(df['Close'].iloc[-1])
            
            # Technical Indicators
            delta = df['Close'].diff()
            gain = (delta.where(delta > 0, 0)).rolling(14).mean()
            loss = (-delta.where(delta < 0, 0)).rolling(14).mean()
            rs = gain / loss
            rsi_series = 100 - (100 / (1 + rs))
            
            current_rsi = rsi_series.iloc[-1]
            current_ema = df['Close'].ewm(span=20).mean().iloc[-1]

            advice, text_col, bg_col = get_ai_advice(current_rsi, last_p, current_ema)
            inr, aed, gold_spot = get_exchange_rates()

            st.metric("Live Wallet Balance", f"₹{st.session_state.balance:,.2f}")

            # AI Advisor Box
            st.markdown(f"""
                <div style="background-color:{bg_col}; padding:20px; border-radius:15px; border: 2px solid {text_col}; margin-bottom: 20px;">
                    <h3 style="color:{text_col}; margin:0;">🚀 Faisal AI Advisor</h3>
                    <p style="color:white; font-size:17px; margin-top:10px;">{advice}</p>
                </div>
            """, unsafe_allow_html=True)

            # Gold Rates (India & Dubai)
            if "Gold" in asset_choice:
                gram_usd = last_p / 31.1035
                pavan_aed = (gram_usd * aed) * 8
                pavan_inr = (gram_usd * inr) * 8
                
                c1, c2 = st.columns(2)
                c1.metric("🇦🇪 Dubai 8g (AED)", f"{pavan_aed:,.2f}")
                c2.metric("🇮🇳 India 8g (INR)", f"₹{pavan_inr:,.2f}")

            # Chart
            fig = go.Figure(data=[go.Candlestick(x=df.index, open=df['Open'], high=df['High'], low=df['Low'], close=df['Close'])])
            fig.update_layout(template="plotly_dark", height=450, margin=dict(l=0,r=0,t=30,b=0))
            st.plotly_chart(fig, use_container_width=True)

    time.sleep(30)
    st.rerun()
